...
class My_ui_2(QWidget, Ui_Form):
    ...
    def paddle_yuce(self, model_src, img):
        ...
        # 预测
        name_list = ["可乐", "绿茶", "蜜茶", "农夫山泉", "冰露"]    # 类
        label = []
        #result = numpy.argmax(results[0])  # 取出预测结果中概率最大的元素索引值
        maxl = 0
        for i,item in enumerate(results[0][0]):
            if item > 0.9:
                maxl = i


        print(name_list[maxl])

        return [[name_list[maxl]]]
    ...
    def insert_table(self, data):
        ...
        if len(data):
            for i, item in enumerate(data):
                sql2 = f"select * from good_list where good_name='{item[0]}'"
                cur.execute(sql2)
    ...
    def pb_sure_img_click(self):
        if len(self.frame) > 0:
            ...
            menu = self.paddle_yuce(paddle_model_src,self.frame)
            # print(menu)
            for item in menu:
    ...
    def pb_start_cam_click(self):
        ...
        while ret:
            ...
            self.frame = frame
            frame = cv2.resize(frame, dsize=(480, 360))  # 可以获取label 大小
...
if __name__ == '__main__':
    ...
    print('-----加载模型-----')
    paddle_model_src = "./model/aistudio/model_final"
    # ------------------------------
    # -----------------------------链接数据库 全局化
    print('-----链接数据库------')
    db = pymysql.connect('localhost', 'root', 'root', 'textdb', charset='utf8')
    cur = db.cursor()
