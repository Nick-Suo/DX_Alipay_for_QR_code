...
db = pymysql.connect(user='root', password='root', database='testdb', charset='utf8')
cur = db.cursor()


def fenxi(pb_txt, dt, db):
    # 菜品数目分析函数
    if os.path.exists('fengxi.png'):
        os.remove('fengxi.png')
    # pb_txt = 'today'
    # dt = ['2020', '5', '9']
    ...
    print(f'\t{good_name[0]} 售出最多，有{fener_max:0.2f}%的占比，{good_name[1]} 售出最少，有{fener_min:0.2f}%的占比')
    # 绘制图
    plt.figure()
